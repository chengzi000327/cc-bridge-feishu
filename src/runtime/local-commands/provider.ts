...
export function isProviderCommand(text: string): boolean {
...
function renderProviderSetMessage(input: { locale: Locale; label: string }): string {
  return input.locale === "zh"
    ? `provider 已设为 ${input.label}。重启此实例后生效。`
    : `Provider set to ${input.label}. Restart this instance to apply.`;
}

export async function handleProviderLocalCommand(
  text: string,
  state: LocalCommandState,
  handlers: LocalCommandHandlers,
): Promise<boolean> {
  const command = parseProviderCommand(text);
  if (!command) return false;

  if (command.kind === "query") {
    await handlers.sendReply(renderCurrentProvider(state.currentProvider, state.locale));
    return true;
  }

  if (command.kind === "invalid") {
    await handlers.sendReply(renderUsage(state.locale));
    return true;
  }

  if (command.kind === "deepseek") {
    const provider = createProviderPreset(state.currentEngine, "deepseek");
    await handlers.updateInstanceConfig((config) => {
      config.provider = provider;
    });
    await handlers.sendReply(renderProviderSetMessage({
      locale: state.locale,
