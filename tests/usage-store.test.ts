  ...
  it("serializes concurrent writes across separate processes", async () => {
    ...
    try {
      await writeFile(scriptPath, [
        `import { UsageStore } from ${JSON.stringify(pathToFileURL(path.join(projectRoot, "src/state/usage-store.ts")).href)};`,
        "(async () => {",
      ...
      await Promise.all([
        execFileAsync(process.execPath, [tsxCliPath, scriptPath, stateDir, "10", "0.1"], projectRoot),
        execFileAsync(process.execPath, [tsxCliPath, scriptPath, stateDir, "20", "0.2"], projectRoot),
      ]);
