  ...
  it("does not lose registrations when multiple processes register concurrently", async () => {
    ...
    try {
      await writeFile(scriptPath, [
        `import { registerInstance } from ${JSON.stringify(pathToFileURL(path.join(projectRoot, "src/bus/bus-registry.ts")).href)};`,
        "(async () => {",
      ...
      await Promise.all([
        execFileAsync(process.execPath, [tsxCliPath, scriptPath, tempDir, "alpha", "9201", "secret-a"], projectRoot),
        execFileAsync(process.execPath, [tsxCliPath, scriptPath, tempDir, "beta", "9202", "secret-b"], projectRoot),
      ]);
